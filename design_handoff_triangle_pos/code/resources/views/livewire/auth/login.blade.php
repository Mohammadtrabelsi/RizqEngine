...
<div class="grid min-h-screen grid-cols-1 lg:grid-cols-[1.1fr_1fr]">
    <div class="flex flex-col justify-between overflow-hidden bg-ink px-16 py-14 text-white">
    ...
    <div class="grid place-items-center bg-canvas px-12 py-14">
        <div class="w-full max-w-[420px]">
            ...
            <a href="{{ route('login.pin') }}" class="block w-full rounded-field border border-hairline bg-white py-3.5 text-center text-[14.5px] font-semibold text-ink transition-colors hover:border-accent hover:text-accent">
                {{ __('auth.use_pin') }}
            </a>

            <p class="mt-7 text-center text-sm text-body">
                {{ __('auth.need_access') }}
                <a href="{{ route('contact.owner') }}" class="font-semibold text-accent hover:text-accent-hover">{{ __('auth.contact_owner') }}</a>
            </p>
        </div>
    </div>
</div>
