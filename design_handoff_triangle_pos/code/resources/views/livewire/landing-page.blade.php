...
<div>
    <header class="sticky top-0 z-40 flex items-center justify-between gap-8 border-b border-hairline bg-canvas/85 px-12 py-[18px] backdrop-blur-md">
        <a href="{{ route('home') }}" class="flex items-center gap-3">
            <x-logo-mark />
            <span class="font-display text-lg font-bold tracking-[-0.01em]">RizqEngine</span>
        </a>

        <nav class="flex items-center gap-[30px] text-sm font-medium text-ink-3">
    ...
    <section class="mx-auto grid max-w-[1440px] grid-cols-1 items-center gap-16 px-12 pb-[88px] pt-24 lg:grid-cols-[1.05fr_0.95fr]">
        <div>
            ...
            <h1 class="mt-6 font-display text-[76px] font-bold leading-[0.98] tracking-display text-balance">
                {{ __('landing.headline') }}
            </h1>
            ...
            <div class="mt-9 flex gap-3">
                <a href="{{ route('login') }}" class="rounded-field bg-ink px-[26px] py-[15px] text-[15px] font-semibold text-white transition-colors hover:bg-ink-2">{{ __('landing.get_started') }}</a>
            ...
            <dl class="mt-14 flex gap-10 border-t border-hairline pt-7">
        ...
        {{-- Till mock: static illustration, safe to keep as markup --}}
        <div class="relative">
            ...
            <div class="absolute -bottom-[26px] -left-7 rounded-card border border-hairline bg-white px-[18px] py-3.5 shadow-mock">
    ...
    <section id="features" class="mx-auto max-w-[1440px] px-12 pb-24 pt-5">
        <div class="flex items-end justify-between gap-10 border-b-2 border-ink pb-6">
            <h2 class="font-display text-[38px] font-bold tracking-[-0.03em]">{{ __('landing.features_title') }}</h2>
        ...
        <div class="mt-px grid gap-px bg-hairline sm:grid-cols-2 lg:grid-cols-3">
            @foreach ($this->features as $i => $feature)
                <article id="{{ $feature['anchor'] ?? null }}" class="bg-canvas px-7 py-8 transition-colors hover:bg-white">
    ...
    <section class="mx-auto max-w-[1440px] px-12 pb-24">
        <div class="flex flex-wrap items-center justify-between gap-12 rounded-panel bg-ink p-12 text-white">
            <div>
                <h2 class="font-display text-[34px] font-bold tracking-[-0.03em]">{{ __('landing.cta_title') }}</h2>
                <p class="mt-3 text-base text-white/70">{{ __('landing.cta_lead') }}</p>
            </div>
            <a href="{{ route('login') }}" class="rounded-field bg-accent px-7 py-4 text-[15px] font-semibold text-white transition-colors hover:brightness-110">
