...
<ul class="c-header-nav ml-auto mr-4">
    ...
    <li class="c-header-nav-item mr-3">
        <a class="btn btn-primary btn-pill {{ request()->routeIs('app.pos.index') ? 'disabled' : '' }}" href="{{ route('app.pos.index') }}">
            <i class="bi bi-cart mr-1"></i> {{ __('pos.pos') }}
        </a>
    </li>
    @endcan
    ...
    <li class="c-header-nav-item dropdown d-md-down-none mr-2">
        ...
        <div class="dropdown-menu dropdown-menu-right dropdown-menu-lg pt-0">
            <div class="dropdown-header bg-light">
                <strong>{{ $low_quantity_products->count() }} {{ __('header.notifications') }}</strong>
            </div>
            @forelse($low_quantity_products as $product)
                <a class="dropdown-item" href="{{ route('products.show', $product->id) }}">
                    <i class="bi bi-hash mr-1 text-primary"></i> Product: "{{ $product->product_code }}" {{ __('header.low-quantity') }}
                </a>
            @empty
                <a class="dropdown-item" href="#">
                    <i class="bi bi-app-indicator mr-2 text-danger"></i> {{ __('header.no-notifications') }}
                </a>
            @endforelse
        </div>
    </li>
        ...
        <a class="c-header-nav-link" data-toggle="dropdown" href="#" role="button"
           aria-haspopup="true" aria-expanded="false">
            ...
            <div class="d-flex flex-column">
                ...
                <span class="font-italic">{{ __('header.online') }} <i class="bi bi-circle-fill text-success" style="font-size: 11px;"></i></span>
            </div>
        </a>
        <div class="dropdown-menu dropdown-menu-right pt-0">
            <div class="dropdown-header bg-light py-2"><strong>{{ __('header.account') }}</strong></div>
