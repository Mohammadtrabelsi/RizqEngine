...
@section('title', __('purchase.edit_payment'))
...
@section('breadcrumb')
    <ol class="breadcrumb border-0 m-0">
        ...
        <li class="breadcrumb-item active">{{ __('purchase.edit_payment') }}</li>
    </ol>
@endsection
                            ...
                            <div class="form-row">
                                <div class="col-lg-4">
                                    <div class="mb-4">
                                        ...
                                        <input type="text" class="block w-full rounded-md border border-slate-300 bg-white px-3 py-2 text-sm leading-normal text-slate-900 placeholder:text-slate-400 transition-colors focus:border-indigo-500 focus:outline-none focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-indigo-600 disabled:opacity-45" name="due_amount" required value="{{ format_currency($purchase->due_amount) }}" readonly>
                                    </div>
                                </div>
                                <div class="col-lg-4">
                                    <div class="mb-4">
                                    ...
                                    <div class="from-group">
                                        <div class="mb-4">
                                            ...
                                            <select class="block w-full rounded-md border border-slate-300 bg-white px-3 py-2 text-sm leading-normal text-slate-900 placeholder:text-slate-400 transition-colors focus:border-indigo-500 focus:outline-none focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-indigo-600 disabled:opacity-45" name="payment_method" id="payment_method" required>
                                                <option {{ $purchasePayment->payment_method == 'Cash' ? 'selected' : '' }} value="Cash">{{ __('purchase.cash') }}</option>
                                                <option {{ $purchasePayment->payment_method == 'Credit Card' ? 'selected' : '' }} value="Credit Card">{{ __('purchase.credit_card') }}</option>
                                                <option {{ $purchasePayment->payment_method == 'Bank Transfer' ? 'selected' : '' }} value="Bank Transfer">{{ __('purchase.bank_transfer') }}</option>
                                                <option {{ $purchasePayment->payment_method == 'Cheque' ? 'selected' : '' }} value="Cheque">{{ __('purchase.cheque') }}</option>
                                                <option {{ $purchasePayment->payment_method == 'Other' ? 'selected' : '' }} value="Other">{{ __('purchase.other') }}</option>
                                            </select>
                                        </div>
                                    </div>
                                </div>
