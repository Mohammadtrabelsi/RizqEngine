                        ...
                        <table class="report-table w-full mb-4 text-slate-900 border-collapse [&_tbody_tr:hover]:bg-slate-50 align-middle mb-0">
                            ...
                            <tbody>
                                @forelse($sales as $sale)
                                    <tr>
                                        <td class="fw-bold" data-label="{{ __('report.reference') }}">{{ $sale->reference }}</td>
                                        <td data-label="{{ __('report.customer') }}"><i class="bi bi-person text-muted"></i> {{ $sale->customer_name }}</td>
                                        <td data-label="{{ __('report.date') }}">{{ \Carbon\Carbon::parse($sale->date)->format('d M, Y') }}</td>
                                        <td class="text-end" data-label="{{ __('report.total') }}">{{ format_currency($sale->total_amount) }}</td>
                                        <td class="text-end text-success" data-label="{{ __('report.paid') }}">{{ format_currency($sale->paid_amount) }}</td>
                                        <td class="text-end @if($sale->due_amount > 0) text-danger fw-bold @else text-muted @endif" data-label="{{ __('report.due') }}">{{ format_currency($sale->due_amount) }}</td>
                                ...
                                @empty
                                    <tr>
                                        <td colspan="8" class="text-center text-muted py-4">{{ __('report.no-sales-data') }}</td>
                                    </tr>
                                @endforelse
                            </tbody>
                        </table>
