                        ...
                        <table class="report-table w-full mb-4 text-slate-900 border-collapse [&_tbody_tr:hover]:bg-slate-50 align-middle mb-0">
                            ...
                            <tbody>
                                @forelse($purchases as $purchase)
                                    <tr>
                                        <td class="fw-bold" data-label="{{ __('report.reference') }}">{{ $purchase->reference }}</td>
                                        <td data-label="{{ __('report.supplier') }}"><i class="bi bi-truck text-muted"></i> {{ $purchase->supplier_name }}</td>
                                        <td data-label="{{ __('report.date') }}">{{ \Carbon\Carbon::parse($purchase->date)->format('d M, Y') }}</td>
                                        <td class="text-end" data-label="{{ __('report.total') }}">{{ format_currency($purchase->total_amount) }}</td>
                                        <td class="text-end text-success" data-label="{{ __('report.paid') }}">{{ format_currency($purchase->paid_amount) }}</td>
                                        <td class="text-end @if($purchase->due_amount > 0) text-danger fw-bold @else text-muted @endif" data-label="{{ __('report.due') }}">{{ format_currency($purchase->due_amount) }}</td>
