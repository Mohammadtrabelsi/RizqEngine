<?php

return [
    '' => '[TODO] ',
    'add_customer' => 'Ajouter un client',
    'address' => 'Adresse',
    'city' => 'Ville',
    'client_type' => 'Type de client',
    'country' => 'Pays',
    'create' => 'Créer',
    'customer_details' => 'Détails du client',
    'customer_email' => 'E-mail du client',
    'customer_name' => 'Nom du client',
    'customer_phone' => 'Téléphone du client',
    'customers' => 'Clients',
    'document' => 'Document descriptif',
    'document_hint' => 'PDF, Word, Excel, texte ou image jusqu\'à 10 Mo.',
    'edit' => 'Modifier',
    'iban' => 'IBAN',
    'legal_entity' => 'Personne morale',
    'legal_entity_tax_required' => 'Un numéro d\'identification fiscale (matricule fiscal) est obligatoire pour les personnes morales.',
    'no_customers_found' => 'Aucun client trouvé',
    'note' => 'Note',
    'physical_person' => 'Personne physique',
    'profile_image' => 'Image du profile',
    'responsible_person' => 'Personne responsable',
    'tax_identification_number' => 'Numéro d\'identification fiscale (NIF)',
    'update' => 'Mettre à jour',
    'whatsapp_number' => 'Numéro WhatsApp',
    'orders' => 'Commandes',
    'payments' => 'Paiements',
    'credit_details' => 'Détails du crédit',
    'account_summary' => 'Résumé du compte',
    'actions' => 'Actions',
    'no_orders' => 'Aucune commande',
    'no_payments' => 'Aucun paiement enregistré',
    'details' => 'Détails',
    'show_payments' => 'Paiements',
    'add_payment' => 'Ajouter un paiement',
    'new_sale' => 'Nouvelle vente',
    'back_to_list' => 'Retour à la liste',
    'total_ordered' => 'Total commandé',
    'total_paid' => 'Total payé',
    'total_due' => 'Total dû',
    'credit_allowed' => 'Ventes à crédit',
    'allowed' => 'Autorisé',
    'not_allowed' => 'Non autorisé',
    'price_list' => 'Liste de prix',
    'back_to_details' => 'Retour aux détails',
    'add_price' => 'Ajouter un prix',
    'product' => 'Produit',
    'select_product' => 'Sélectionner un produit',
    'price' => 'Prix',
    'negotiated_prices' => 'Prix négociés',
    'default_price' => 'Prix par défaut',
    'no_negotiated_prices' => "Aucun prix négocié pour ce client pour l'instant.",
    'confirm_delete_price' => 'Supprimer ce prix négocié ?',
    'price_saved' => 'Prix enregistré.',
    'price_deleted' => 'Prix supprimé.',
    'delete' => 'Supprimer',
];
